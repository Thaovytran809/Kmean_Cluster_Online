d3.json("/customer-data/").then(data => {
  const mntCols = ['mnt_wines', 'mnt_fruits', 'mnt_meat_products', 'mnt_fish_products', 'mnt_sweet_products', 'mnt_gold_prods'];

  // Lọc bỏ dữ liệu có cluster không hợp lệ
  const filteredData = data.filter(d => d.cluster != null && !isNaN(d.cluster));

  // Gom dữ liệu theo cluster
  const nestedData = d3.group(filteredData, d => d.cluster);
  
  const clusterMeans = Array.from(nestedData, ([cluster, values]) => {
    const means = {};
    mntCols.forEach(col => {
      means[col] = d3.mean(values, v => +v[col] || 0);
    });
  ...
  function drawGroupedBarChart(svgId, data) {
    ...
    g.append("g")
      ...
      .join("rect")
        ...
        .on("mouseover", (event, d) => {
          ...
          tooltip.html(`${d.cluster}<br>Chi tiêu TB: ${d.value.toFixed(2)}`)
    ...
    g.append("g")
      ...
      .call(d3.axisBottom(x0).tickFormat(d => d.replace('mnt_', '').replace('_products', '')))
      .selectAll("text")
    ...
    .text("Chi tiêu trung bình theo cụm");

    // Nhãn trục Y
    // g.append("text")
    //   .attr("x", -40)
    //   .attr("y", -10)
    //   .text("Chi tiêu trung bình")
