const margin4 = { top: 40, right: 30, bottom: 70, left: 60 },
      totalWidth = 900,
      totalHeight = 350,
      chartWidth = (totalWidth - margin4.left - margin4.right - 80) / 2,
...
const tooltip = d3.select("body").append("div")
  ...
  d3.json("/customer-data/").then(rawData => {
    const data = rawData.filter(d => d.cluster != null); // ⚠️ lọc bỏ cluster null
  ...
  const eduTexts = eduBars.selectAll("text")
  .data(d => d.map(item => ({
...
// Thêm 2 dòng bằng tspan cho mỗi text
eduTexts.each(function(d) {
  ...
  // Add labels to show category and percentage
  const texts = marBars.selectAll("text")
  .data(d => d.map(item => ({
