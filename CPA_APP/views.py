...
def perform_rfm_clustering(request):
    ...
    customers = Customer.objects.all().values(
        ...
        'num_web_purchases', 'num_catalog_purchases', 'num_store_purchases', 'education', 
        'marital_status','num_web_visits_month'
    )
...
def customer_segments(request):
    # Lấy tất cả khách hàng và nhóm theo cluster, loại bỏ cluster=None
    customers = Customer.objects.exclude(cluster=None).order_by('cluster')  # Loại bỏ các khách hàng có cluster = None
    # Nhóm khách hàng theo cluster
    grouped_customers = {}
    # Lặp qua các cluster và phân trang cho mỗi cluster
    for cluster in customers.values('cluster').distinct():  # Lấy từng cụm
        cluster_customers = customers.filter(cluster=cluster['cluster'])
        # Phân trang cho từng cụm, mỗi trang có 20 khách hàng
        paginator = Paginator(cluster_customers, 20)  # 20 khách hàng mỗi trang
        page_number = request.GET.get('page')  # Lấy số trang từ URL
        page_obj = paginator.get_page(page_number)  # Lấy trang hiện tại
        
        # Lưu các khách hàng của từng cụm vào grouped_customers
        grouped_customers[cluster['cluster']] = {
            'page_obj': page_obj,  # Lưu thông tin phân trang
            'total_pages': paginator.num_pages  # Số trang của cụm đó
        }
...
def customer_data_json(request):
    customers = Customer.objects.all().values(
        ...
        'num_web_purchases', 'num_catalog_purchases', 'num_store_purchases','recency','education','marital_status'
    )
...
from .models import ClusterDescription

@csrf_exempt
def save_cluster_description(request):
    if request.method == "POST":
        cluster = request.POST.get("cluster")
        description = request.POST.get("description")

        if not cluster or not description:
            return JsonResponse({"status": "error", "message": "Thiếu dữ liệu"})

        obj, _ = ClusterDescription.objects.update_or_create(
            cluster_id=cluster,
            defaults={"description": description}
        )
        return JsonResponse({"status": "success", "message": "Đã lưu mô tả cụm thành công"})
